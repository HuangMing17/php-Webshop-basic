...
class ProductModel
{
    ...
    {
        $query = "SELECT p.id, p.name, p.description, p.SoLuong , p.price, p.image, c.name as
category_name
...
WHERE p.id = :id";
    ...
    public function addProduct($name, $description, $price, $category_id, $image, $SoLuong)
    {
        ...
        if (!is_numeric($SoLuong) || $SoLuong < 1) {
            $errors['SoLuong'] = 'Số Lượng Không Thể Bé Hơn 1';
        }
    ...
    category_id, image, SoLuong) VALUES (:name, :description, :price, :category_id, :image, :SoLuong)";
    ...
    image=:image, SoLuong=:SoLuong WHERE id=:id";
